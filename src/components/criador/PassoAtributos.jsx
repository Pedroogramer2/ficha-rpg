...
// 👇 BASE DE PROTEÇÃO ANTI-NAN 👇
const BASE_SEGURA = { forca: 8, destreza: 8, constituicao: 8, inteligencia: 8, sabedoria: 8, carisma: 8 };
...
export function PassoAtributos() {
  ...
  const getBonusTalentos = useCallback((chaveAttr) => {
    if (!dados.talentos) return 0;
    let totalBonus = 0;
    const chaveLimpa = chaveAttr.toLowerCase(); 
    dados.talentos.forEach(talento => {
      if (talento.bonusAtributos) {
        totalBonus += talento.bonusAtributos.filter(attr => attr.toLowerCase() === chaveLimpa).length;
      }
    });
    return totalBonus;
  }, [dados.talentos]);
  ...
  // 👇 GARANTE QUE SE O SAVE TIVER FALTANDO UM ATRIBUTO, ELE PÕE O 8! 👇
  const valoresSimplesRender = useMemo(() => {
    if (!dados.valoresSimplesSalvos) return BASE_SEGURA;
    return { ...BASE_SEGURA, ...dados.valoresSimplesSalvos };
  }, [dados.valoresSimplesSalvos]);
  ...
  useEffect(() => {
    ...
    if (metodo === 'manual' || metodo === 'pointbuy') {
      ATRIBUTOS.forEach(k => {
        const valSeguro = Number(valoresSimplesRender[k]) || 0;
        resultadoFinal[k] = valSeguro + getBonusAntecedente(k) + getBonusTalentos(k);
      });
    } else {
      ...
      ATRIBUTOS.forEach(attr => {
        ...
        resultadoFinal[attr] = valorBase + getBonusAntecedente(attr) + getBonusTalentos(attr); 
      });
    }
    
    // 👇 QUEBRA DE LOOP 👇
    atualizar(prev => {
      const prevAttrs = prev.atributos || {};
      const mudouAlgo = ATRIBUTOS.some(attr => {
        if (Number.isNaN(prevAttrs[attr]) && Number.isNaN(resultadoFinal[attr])) return false;
        return prevAttrs[attr] !== resultadoFinal[attr];
      });
      
      if (!mudouAlgo) return prev; 
      
      return { 
        ...prev, 
        forca: resultadoFinal.forca, destreza: resultadoFinal.destreza, constituicao: resultadoFinal.constituicao,
        inteligencia: resultadoFinal.inteligencia, sabedoria: resultadoFinal.sabedoria, carisma: resultadoFinal.carisma,
        atributos: resultadoFinal 
      };
    });
  }, [valoresSimplesRender, alocacoes, rolagens, metodo, getBonusAntecedente, getBonusTalentos, atualizar]);

  function setMetodo(m) {
    if (metodo !== m) atualizar(prev => ({ ...prev, metodoCriacaoAtributos: m, alocacoesSalvas: {} }));
  }
  ...
  function alocar(atributo, indexDoArray) {
    atualizar(prev => ({ ...prev, alocacoesSalvas: { ...prev.alocacoesSalvas, [atributo]: indexDoArray } }));
  }

  function mudarPointBuy(attr, delta) {
    const val = valoresSimplesRender[attr];
    const novo = val + delta;
    if (novo < 8 || novo > 15) return;
    
    let gasto = 0;
    Object.keys(valoresSimplesRender).forEach(k => gasto += CUSTO[valoresSimplesRender[k]]);
    const diferenca = CUSTO[novo] - CUSTO[val];
    if (gasto + diferenca > 27) return;

    atualizar(prev => ({ 
      ...prev, 
      valoresSimplesSalvos: { ...(prev.valoresSimplesSalvos || BASE_SEGURA), [attr]: novo } 
    }));
  }
  ...
  const getValorBaseDisplay = (attr) => {
    if (metodo === 'manual' || metodo === 'pointbuy') return valoresSimplesRender[attr];
    const pool = metodo === 'padrao' ? ARRAY_PADRAO : rolagens;
    const idx = alocacoes[attr];
    return (idx !== undefined && idx !== "" && pool[idx] !== undefined) ? pool[idx] : "-";
  };

  const pontosGastos = Object.values(valoresSimplesRender).reduce((acc, v) => acc + (CUSTO[v]||0), 0);
  ...
  return (
    <div className="passo-atributos-container">
      <div className="abas-metodo">
        {['padrao', 'pointbuy', 'rolagem', 'manual'].map(m => (
          <button key={m} className={metodo === m ? 'ativo' : ''} onClick={() => setMetodo(m)}>
            {m === 'padrao' ? "Padrão" : m === 'pointbuy' ? "Point Buy" : m === 'rolagem' ? "Rolagem" : "Manual Livre"}
          </button>
        ))}
      </div>
      ...
      <div className="area-pool-info">
        ...
        {/* 👇 AQUI ESTAVA O BUG DO VITE (< 0). CORRIGIDO PARA > 27 👇 */}
        {metodo === 'pointbuy' && (
          <div className="pb-info">Pontos: <strong style={{ color: pontosGastos > 27 ? 'red' : '#ffcc00' }}>{27 - pontosGastos}</strong> / 27</div>
        )}
                ...
                <div className="input-area-atrib">
                  ...
                  {metodo === 'manual' && (
                    <input 
                      ...
                      value={valoresSimplesRender[attr]}
                      onChange={(e) => {
                        const novoVal = parseInt(e.target.value) || 0;
                        atualizar(prev => ({
                          ...prev,
                          valoresSimplesSalvos: { ...(prev.valoresSimplesSalvos || BASE_SEGURA), [attr]: novoVal }
                        }));
                      }}
                    />
                  )}
